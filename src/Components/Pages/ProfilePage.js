...
const ProfilePage = async () => {
    ...
    if (!idCurrentUser || isNaN(idCurrentUser)) {
        pageDiv.innerHTML = `<p>Aucun profil à afficher</p>`;
        return;
    }
    ...
    try {
        ...
        const responsePostsInfo = await fetch("/api/posts/user/" + idCurrentUser, request); // fetch return a promise => we wait for the response
        if (!responsePostsInfo.ok) {
            // status code was not 200, error status code
            throw new Error(
                "fetch error : " + responsePostsInfo.status + " : " + responsePostsInfo.statusText
            );
        }
        ...
        const user = await responseUserInfo.json();
        const posts = await responsePostsInfo.json();
        const likes = await responseLikes.json();
        let date = new Date(user.date_creation);
        let likesPost = 0;
        let dateString = date.toDateString();
